 ...
 */
class Delivery_Groups_Ajax extends \Oxid_Esales\Eshop\Application\Controller\Admin\List_Component_Ajax
{
    /**
     * Columns array
     *
    ...
    protected function get_query()
    {
        ...
        if (!$s_id) {
            ...
            $s_q_add = " from oxobject2delivery left join {$s_group_table} " . "on {$s_group_table}.oxid=oxobject2delivery.oxobjectid " . ' where oxobject2delivery.oxdeliveryid = ' . $o_db->quote($s_id) . " and oxobject2delivery.oxtype = 'oxgroups' ";
        }
        if ($s_synch_id && $s_synch_id != $s_id) {
            $s_q_add .= " and {$s_group_table}.oxid not in ( select {$s_group_table}.oxid " . "from oxobject2delivery left join {$s_group_table} " . "on {$s_group_table}.oxid=oxobject2delivery.oxobjectid " . ' where oxobject2delivery.oxdeliveryid = ' . $o_db->quote($s_synch_id) . " and oxobject2delivery.oxtype = 'oxgroups' ) ";
        }
        return $s_q_add;
    }
    /**
     ...
     */
    public function remove_group_from_del(): void
    {
     ...
     */
    public function add_group_to_del(): void
    {
        ...
        if ($sox_id && $sox_id != '-1' && is_array($a_chosen_cat)) {
            foreach ($a_chosen_cat as $s_chosen_cat) {
                ...
                $o_object2delivery->oxobject2delivery__oxtype = new \Oxid_Esales\Eshop\Core\Field('oxgroups');
