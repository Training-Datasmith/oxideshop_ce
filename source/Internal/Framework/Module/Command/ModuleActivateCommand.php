 ...
 */
class Module_Activate_Command extends Command
{
    ...
    {
        $this->set_description('Activates a module.')->add_argument('module-id', Input_Argument::REQUIRED, 'Module ID')->set_help('Command activates module by defined module ID.');
    }
    protected function execute(Input_Interface $input, Output_Interface $output): int
    {
        $module_id = $input->get_argument('module-id');
        if ($this->is_installed($module_id)) {
            $this->activate_module($output, $module_id);
        } else {
            $output->write_ln('<error>' . sprintf(static::MESSAGE_MODULE_NOT_FOUND, $module_id) . '</error>');
        }
        return 0;
    }
    protected function activate_module(Output_Interface $output, string $module_id)
    {
        $this->module_activation_service->activate($module_id, $this->context->get_current_shop_id());
        $output->write_ln('<info>' . sprintf(static::MESSAGE_MODULE_ACTIVATED, $module_id) . '</info>');
    }
