...
class Activation_Dependency_Validator implements Module_Configuration_Validator_Interface
{
    ...
    {
        $unresolved_dependencies = $this->module_dependency_resolver->get_unresolved_activation_dependencies($configuration->get_id(), $shop_id);
        if (!$unresolved_dependencies->has_module_dependencies()) {
            return;
        }
        throw new Dependency_Validation_Exception(sprintf('Module "%s" has unfulfilled dependencies in shop "%d" and can not be activated. 
